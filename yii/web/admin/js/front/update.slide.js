...
function findImage() {
  ...
  $.ajax({
    url: 'uploads/front-slides/' + $("meta[name=slide]").attr("content") + '.200.200.jpg',
    type: 'HEAD',
    cache: false,
    error: function() {
      $('#article-photo').hide();
    ...
    success: function() {
      $('#article-photo').attr('src', 'uploads/front-slides/' + $("meta[name=slide]").attr("content") + '.200.200.jpg?t=' + (new Date()).toString());
      $('#article-photo').show();
    }
  });
}
...
Dropzone.options.mydropzone = {
  paramName: "imageFile",
  url: $("meta[name=path]").attr("content") + '/front-slides/image',
  maxFilesize: .5, // MB
